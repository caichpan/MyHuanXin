...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
   ...
   //     chatBtn.titleLabel.lineBreakMode = UILineBreakModeWordWrap;
        [chatBtn addTarget:self action:@selector(chatBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        chatBtn.titleLabel.textAlignment = NSTextAlignmentLeft;
        chatBtn.titleLabel.numberOfLines = 0;
        // 需要设置内容的内边距
       // chatBtn.titleEdgeInsets = UIEdgeInsetsMake(0, 0, 10, 0);
       self.chatButton.contentEdgeInsets = UIEdgeInsetsMake(15, 20, 25, 20);
...
- (void)layoutSubviews
{
    ...
    self.chatTime.frame = CGRectMake(0, 0, ChatScreenWidth, 30);
}
...
- (void)setMessage:(EMMessage *)message
{
 ...
 //   self.chatTime.text = [self conversationTime:message.timestamp];
    ...
    if ([body isKindOfClass:[EMTextMessageBody class]]) {// 文本类型
        ...
        CGSize size = [textBody.text boundingRectWithSize:CGSizeMake(ChatScreenWidth/2, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15.0f]} context:nil].size;
        ...
        [self.chatButton yy_setImageWithURL:url forState:UIControlStateNormal placeholder:nil];
//        [self.chatButton sd_setImageWithURL:url forState:UIControlStateNormal];
    ...
    if ([message.from isEqualToString:chatter]) {// 自己发送的
        ...
        // 头像在右边
        self.chatIcon.frame = CGRectMake(ChatScreenWidth - kWeChatAllSubviewHeight - kWeChatPadding, 30 + kWeChatPadding, kWeChatAllSubviewHeight, kWeChatAllSubviewHeight);
        
        // 聊天消息是左边
        self.chatButton.left = ChatScreenWidth - self.chatButton.width - self.chatIcon.width - kWeChatPadding*2;
